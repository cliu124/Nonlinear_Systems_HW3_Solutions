...
degree=8; %polynomial degree of V function
...
f=([x(2);-x(1)-x(2)-(2*x(2)+x(1))*(1-x(2)^2)]);%RHS of nonlinear term. 
...
theta_list=linspace(0,2*pi,50);
for theta_ind=1:length(theta_list)
    theta=theta_list(theta_ind);
    x0=0.001*[cos(theta);sin(theta)]; %initial condition close to origin. 
    [t,z]=ode45(@(t,z) -[z(2);-z(1)-z(2)-(2*z(2)+z(1))*(1-z(2)^2)],[0,20],x0);  %note that we reverse the time. 
    plot(z(:,1),z(:,2),'k','LineWidth',1); hold on;
end

xlabel('x_1'); ylabel('x_2');
saveas(gcf,['HW3_prob3_degree=',num2str(degree),'.png']);